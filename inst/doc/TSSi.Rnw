...
was mapped to the genome, such that the number of sequenced 5' end
...
chromosome, the strand, the 5' position of the reads, and the total
number of mapped. Further, regions based on existing annotation are
also provided which are used to divide the data into independent subsets
for analysis.
...
For details about the method, please refer to the future publication:\\
\emph{TSSi - An R package for transcription start site identification
from 5}'\emph{ mRNA tag data}\\
\emph{C. Kreutz, J. Gehring, D. Lang, J. Timmer, and S. A. Rensing}
