...
for each position are available%
\footnote{The sequencing workflow at the bioconductor website describes basic
steps and tools for the import and processing of sequcencing data
(\url{http://bioconductor.org/help/workflows/high-throughput-sequencing/}).%
}. The data frame \Rvar{readData} contains the chromosome, the strand,
the 5' position of the reads, and the total number of reads at that
position. Further, regions based on existing annotation are also provided
which are used to divide the data into independent subsets for analysis.
